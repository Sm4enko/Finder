...
void Spider::crawl_page(const std::string url, int depth, std::string data, bool isThread) {
...
std::string Spider::remove_html_tags(const std::string& input) {
	std::string output;
...
std::string Spider::remove_punctuation(const std::string& input) {
	std::string output;
...
std::string Spider::UTF8_to_CP1251(std::string const& utf8)
{
	if (!utf8.empty())
	{
...
int Spider::printMsg(std::string msg) {
	try {
		std::mutex my_mutex;

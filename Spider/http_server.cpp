...
std::string make_response(const std::string request) {
	...
	create_table();
	crawl_page(settings["StartPage"], searchWords, std::stoi(settings["RecursionDepth"]), data, 0, 0);
...
void handle_request(tcp::socket &socket) {
	...
	if (request.find("GET /") != std::string::npos) {
		...
		std::ifstream file("D:/Finder/C++/Finder/index.html");
		if (file) {
...
void start_http_server() {
	...
	//		boost::thread t(boost::bind(handle_request,std::ref(socket)));// handle_request(socket);
	//	}
	//}
